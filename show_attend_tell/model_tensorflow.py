...
def train(pretrained_model_path=pretrained_model_path): #This function trains the model
...
def test(test_feat='./data/guitar_player.npy', model_path='./model/model-990', maxlen=20):
    annotation_data = pd.read_pickle(annotation_path)
    captions = annotation_data['caption'].values
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)
    n_words = len(wordtoix)
...
#This function tests the model using tensor_flow
def test_tf(test_image_path='./images/1015118661_980735411b.jpg', model_path='./model/model-990', maxlen=30):
    ...
    ixtoword = np.load('./data/feats.npy').tolist()
